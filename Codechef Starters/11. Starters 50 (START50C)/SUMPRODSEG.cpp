...
void solve() {
    ...
    bool flag = false;
    pair<int,int> segmentOne, segmentTwo;
    
    for(auto&i:FACTOR) {
        if(i<=y/i) {
            segmentTwo = {i, y/i};
            segmentOne = {x-(i-1), i-1};
            
            if(segmentOne.first <= segmentOne.second) {
                flag = true;
                break;
            }
            
            segmentOne = {y/i+1, x-(y/i+1)};
    ...
    if(flag)
        display(segmentOne.first<<" "<<segmentOne.second<<endl<<segmentTwo.first<<" "<<segmentTwo.second)
    else
        display("-1")
}
